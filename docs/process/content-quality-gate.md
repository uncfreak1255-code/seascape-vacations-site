...
Any PR that changes `reader copy` in `src/` must complete these editorial steps
before review or merge. Use native reasoning with the active brief and
`docs/style/`; no global writing skill is required:

1. **Draft the copy** around the reader decision, argument, and CTA
2. **Remove internal wording** to strip repo notes, internal labels, and process-heavy wording out of visible copy
3. **Check voice and specificity** for the final anti-slop pass on rhythm, fake specificity, and obvious AI texture
...
After the copy chain, the existing Voice Editor must return `Approved`.
`Reject` or `Needs rewrite` blocks human review, push, and merge. The verdict
applies to static page copy, copy generated by interactive JavaScript, and
reader-facing component configuration such as `guideConversionKit`.

The final voice pass removes AI texture; it does not get to remove answer
structure. On guide, stay, and research pages — the routes that get cited in
AI answers and snippets — the final voice step may not dissolve the standalone
...
- Keep internal funnel shorthand out of reader copy. `Trip shape`, `stay base`,
