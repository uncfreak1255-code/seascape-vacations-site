...
## Visual regression baselines (Card 1, 2026-06-13)

- Playwright visual baselines are renderer/OS-specific: a baseline committed from a local Linux container will not match the macOS CI runner (font hinting and anti-aliasing differ), so `visual-regression` fails on pixel diff even when the change is correct. This bit Card 1 (PR #331) — the locally regenerated baseline failed CI until it was regenerated to match the macOS renderer. Treat the CI renderer as the baseline source of truth: regenerate baselines for/in the CI environment (or update them from the CI run), not just from the local container, when a visual change is intended.
