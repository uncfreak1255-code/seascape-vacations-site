...
- `www.seascapevacations.com` currently returns a `/lander` shell for owner funnel routes while canonical `seascape-vacations.com` serves the real pages; keep `npm run verify:owner-funnel-routes` as the live canary until the external host/DNS/Netlify mapping is fixed
