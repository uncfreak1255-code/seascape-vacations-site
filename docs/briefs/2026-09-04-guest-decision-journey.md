...
The broader CI run exposed duplicate 800px/1200px hero downloads on two existing AMI stay pages, plus an eager 1400px card image. Matching the preload to the actual CSS image and restoring existing 800px/1400px card candidates reduced initial transfer from about 743KB to 473KB. Three-run mobile median LCP fell from 4.85s/4.82s to 3.53s/3.54s, passing the same 4.5s budget. Photos and layout are unchanged; four affected visual/accessibility checks passed without new baselines. The full normal-data proof now captures 15 routes on both devices; its local image-CDN handling fixes the owner's otherwise-404 hero using the exact source photo. Logs: /private/tmp/seascape-stays-after.log, /private/tmp/seascape-stay-visual-final.log and /private/tmp/seascape-full-proof-final.log.

Dependency triage found six inherited development-tool alerts; package manifests are unchanged and no new deployed exposure was identified. Handle dependency upgrades as separate maintenance, including sharp/js-yaml/ip-address and the Puppeteer archive dependency.
