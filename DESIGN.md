...
The 51 pre-Waterline guide routes, the owner pages and the research pages ship bespoke styling, including a gold pill CTA (`#C9A962`). It rendered white-on-gold at 2.25:1 against a 4.5:1 requirement, on hit boxes as small as 40px, and it was the owner funnel's primary CTA as well as the guides'. That gold is retired. Every gold button uses the Waterline primary button: ink with white text on cream, white, the old teal and photos, and citron with ink text on the two sticky CTA bars and the near-black owner hero, where ink drops to 1.51:1. Radius is 4px, never a pill. The overrides live in `src/css/guest.css` and need `!important` because these pages carry the colours inline. The tracked owner CTA is selected by its `data-track-event` attribute rather than the generic `.btn` class, which 62 built pages use for something already correct. Gold survives as a non-clickable accent on borders, badges and card rules; only the buttons changed.
