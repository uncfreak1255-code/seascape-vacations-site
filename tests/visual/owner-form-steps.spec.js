...
test("owner form step controls use high-contrast ink and keep gold decorative", async ({ page }) => {
  ...
  const active = page.locator(".owner-field-step-dot.is-active");
  ...
  await expect(page.locator(".owner-field-step-dot.is-active")).toHaveAttribute("aria-label", "Go to step 2");

  const completeColors = await readColors(page.locator(".owner-field-step-dot.is-complete"));
  assert.equal(completeColors.color, WHITE);
  assert.equal(completeColors.background, INK);

  const nextActive = await readColors(page.locator(".owner-field-step-dot.is-active"));
  assert.equal(nextActive.color, WHITE);
  assert.equal(nextActive.background, INK);

  const completeLine = page.locator(".owner-field-step-line.is-complete").first();
