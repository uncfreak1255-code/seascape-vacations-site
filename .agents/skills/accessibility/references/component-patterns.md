...
Examples for the affected component, not a replacement design system. Use existing
Seascape styles and native controls. Example modal code illustrates focus handling;
production dialogs also need initial focus, an accessible name, and focus return.
...
|-----------|------------|--------------|
| Normal text (below large-text threshold) | 4.5:1 | 7:1 |
...
**Use native buttons for actions; they already support Enter and Space.**
Only add explicit keyboard handlers to custom controls that need them:
```javascript
// ❌ Only handles click
