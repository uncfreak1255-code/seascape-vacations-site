...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--preview") options.preview = true;
      ...
      options.pricePerMillionInputTokens = Number(argv[++index]);
  ...
  if (
    options.pricePerMillionInputTokens !== null &&
    (!Number.isFinite(options.pricePerMillionInputTokens) || options.pricePerMillionInputTokens < 0)
  ) {
...
function defaultOutputDir() {
  ...
  return path.join(os.tmpdir(), `seascape-aeo-evaluation-${stamp}`);
}
...
function sourceState(goldenResults) {
  const evidenceFiles = [
    "scripts/evals/evals.config.json",
    lane.rubric,
    "scripts/evals/run-jev-aeo-trial.js",
  ...
  try {
    return {
      ...
      evaluationDigest,
      evidenceFiles,
      dirty: execFileSync("git", ["status", "--porcelain", "--", ...evidenceFiles], {
        cwd: projectRoot,
        encoding: "utf8",
      }).trim().length > 0,
    };
...
function renderMarkdown(findings) {
  const lines = [
    ...
    `- Source commit: \`${findings.source.commit}\``,
    `- Evaluation digest: \`${findings.source.evaluationDigest}\``,
    `- Relevant source dirty: \`${findings.source.dirty}\``,
    `- Requested model: \`${findings.provider.modelRequested}\``,
    `- Returned model: \`${findings.provider.modelReturned || "not called"}\``,
    `- Credential lifecycle: \`${findings.credentialLifecycle}\``,
    "",
    "## Payload manifest",
    "",
    "| Fixture | Bytes | SHA-256 | Judgments |",
    "|---|---:|---|---:|",
    ...findings.payload.fixtures.map(
      (fixture) => `| ${fixture.name} | ${fixture.copyBytes} | \`${fixture.copySha256}\` | ${fixture.judgmentCount} |`
    ),
  ];
  ...
  if (findings.results?.length > 0) {
    lines.push(
      ...
      "## Results",
      "",
      "| Fixture | Expected | Score | Match | Mean confidence | Input tokens | Latency |",
      "|---|---|---:|---|---:|---:|---:|",
  ...
  if (findings.summary) {
    lines.push(
      ...
      "## Summary",
      "",
      `- Fixture matches: ${findings.summary.matches}/${findings.summary.fixtures}`,
      `- Input tokens: ${findings.summary.inputTokens}`,
      `- Estimated input cost: ${formatUsd(findings.summary.costUsd)}`,
      `- Total request latency: ${findings.summary.latencyMs} ms`,
      `- Decision: \`${findings.summary.decision}\``,
      "",
...
async function run(argv = process.argv.slice(2), dependencies = {}) {
  ...
  const base = {
    ...
    source: sourceState(goldenResults),
    ...
    provider: {
      ...
      modelReturned: null,
      inputCostPerMillionUsd: options.pricePerMillionInputTokens,
    },
    payload: {
      scope: "three non-guest repository AEO golden fixtures",
      externalData: "fixture copy plus five rubric-derived Score questions",
      fixtures: fixtureManifest(goldenResults, Object.keys(questions).length),
    },
  };
  ...
  base.provider.modelReturned = [...new Set(results.map((result) => result.model))].join(", ");
  base.results = results;
  base.summary = {
    ...
    costUsd:
      options.pricePerMillionInputTokens === null
        ? null
        : results.reduce((sum, entry) => sum + entry.costUsd, 0),
